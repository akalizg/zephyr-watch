...
public class OnlineInferenceJob {
    ...
    public static void main(String[] args) throws Exception {
        ...
        boolean debugPrint = args.length > 2
        ...
        SingleOutputStreamOperator<SensorReading> cleanStream = env
            .fromSource(
        ...
        if (debugPrint) {
            ...
            cleanStream
                .getSideOutput(ParseAndValidateSensorProcessFunction.INVALID_SENSOR_TAG)
        ...
        riskStream
            .addSink(MySqlSinkFactory.buildRiskPredictionSink())
        ...
        PatternStream<RiskPrediction> patternStream = CEP.pattern(
            riskStream.keyBy(RiskPrediction::getMachineId),
            consecutiveRiskPattern
        );

        SingleOutputStreamOperator<AlertEvent> cepAlertStream = patternStream
        ...
        alertStream
            .addSink(new WebhookAlertSink())
            .name("Webhook_Alert_Sink");

        env.execute(JobConfig.ONLINE_INFERENCE_JOB_NAME);
    }

    private static class HighRiskLabelCondition extends SimpleCondition<RiskPrediction> {
        @Override
