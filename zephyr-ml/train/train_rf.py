...
def build_metadata(args, train_df: pd.DataFrame, test_df: pd.DataFrame) -> Dict[str, Any]:
    return {
        "modelVersion": "pmml-local-rul-v1",
...
def main():
    ...
    model_metadata_path = os.path.join(args.artifact_dir, "model_metadata.json")
    model_metadata = {
        "modelVersion": metadata["modelVersion"],
        "modelType": metadata["modelType"],
        "modelUri": "model.pmml",
        "fallbackModelUri": "model.pkl",
    ...
    feature_columns_path = os.path.join(args.artifact_dir, "feature_columns.json")
    with open(feature_columns_path, "w", encoding="utf-8") as f:
        json.dump({"featureColumns": FEATURE_COLUMNS}, f, ensure_ascii=False, indent=2)

    threshold_path = os.path.join(args.artifact_dir, "threshold.json")
    thresholds = {
        "riskAlertThreshold": 0.70,
        "riskCriticalThreshold": 0.90,
        "rulWarningThreshold": 60.0,
        "rulCriticalThreshold": 30.0,
        "riskStrategy": "rul_logistic_with_feature_volatility"
    }
