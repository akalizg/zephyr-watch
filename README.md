...
| 类别 | 内容 |
|---|---|
| 已完成 | Kafka 数据接入；Flink 清洗与窗口特征工程；REST 模型服务在线推理；Flink 在线推理；阈值告警；多规则特征异常告警；CEP 连续高风险告警；MySQL/Redis/Kafka 输出；规则 + 历史案例相似度维修推荐；Spring Boot Dashboard API；轻量 Dashboard 页面；Grafana 业务 SQL 与 Dashboard JSON；人工审核回流基础链路；多模型训练报告与简单加权融合；MinIO 模型注册；增量训练闭环 |
| 部分完成 | PMML 本地推理；Webhook 真实推送配置。 |
| 规划扩展 | ONNX Runtime；TF Serving；Prometheus 系统级监控；Two-Tower / LightGCN / SASRec 深度推荐；自动灰度发布和回滚；CI/CD。 |
...
- README 与真实代码完全同步
- 模型产物生成规范
- Prometheus 系统监控 Dashboard（后续扩展）
- Hive ADS 层
- Webhook 配置测试发送、删除、风险级别过滤
- 分类模型 PMML 导出；ONNX 导出为后续扩展
- Linux / Mac 启动脚本
