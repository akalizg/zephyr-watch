...
| 类别 | 内容 |
|---|---|
| 已完成 | Kafka 数据接入；Flink 清洗与窗口特征工程；REST 模型服务在线推理；Flink 在线推理；阈值告警；多规则特征异常告警；CEP 连续高风险告警；MySQL/Redis/Kafka 输出；规则 + 历史案例相似度维修推荐；Spring Boot Dashboard API；轻量 Dashboard 页面；Grafana 业务 SQL 与 Dashboard JSON；人工审核回流基础链路；多模型训练报告与简单加权融合。 |
| 部分完成 | HDFS/Hive 离线链路；MinIO 模型注册；PMML 本地推理；增量训练闭环；Webhook 真实推送配置。 |
