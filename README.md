...
| 类别 | 内容 |
|---|---|
| 已完成 | Kafka 数据接入；Flink 清洗与窗口特征工程；REST 风险模型服务在线推理；PMML RUL 本地推理；Flink 在线推理；阈值告警；多规则特征异常告警；CEP 连续高风险告警；MySQL/Redis/Kafka 输出；在线特征快照；规则 + 历史案例相似度维修推荐；Spring Boot Dashboard API；轻量 Dashboard 页面；Grafana 业务 SQL 与业务 Dashboard JSON；人工审核回流基础链路；反馈训练样本生成；多模型训练报告与简单加权融合；MinIO 模型注册；风险模型服务基础热加载；增量训练闭环；Windows 一键启动脚本；Webhook（`GENERIC`/企业微信/钉钉）与 `webhook_send_log`；Spring Boot Prometheus 与任务 C 抓取配置及系统看板 |
| 部分完成 | 自动周期增量训练入口；模型注册后的人工激活流程；无效传感器 Side Output 预留。 |
| 未完成扩展| ONNX Runtime 在线推理接入；TF Serving 推理接入；模型灰度发布与自动回滚；生产级自动调度与热加载增强；Flink Async I/O REST 推理；无效传感器数据落 Kafka 链路。 |
...
| 模型部署 | REST / PMML；ONNX / TF Serving 为后续扩展 | 将离线训练模型部署到实时推理链路 |
...
| 在线推理 | Flink 通过 REST 调用风险模型服务，PMML/JPMML 用于 RUL 本地推理；ONNX 和 Async I/O 为后续扩展 |
...
当前在线推理主链路采用 Flink 同步调用 Flask REST 模型服务的方式完成风险分类推理。模型服务默认加载 `best_risk_model.pkl`、`threshold.json`、`feature_columns.json` 和 `model_metadata.json`，输出 `riskProbability`、`riskLabel`、`riskLevel` 等结果。PMML/JPMML 已用于 RUL 本地推理；风险分类模型的 ONNX、TF Serving 和 Flink Async I/O 调用仍属于后续增强方向。
...
| REST 风险模型服务 | Flink 同步 Map + Flask API | 当前风险分类主链路 |
| Flink Async I/O REST 推理 | Flink Async I/O + Flask API | 后续性能增强 |
| TF Serving | TensorFlow Serving | 后续深度学习模型服务扩展 |
...
- Kafka risk_prediction_topic、alert_event_topic、review_label_topic
...
- 无效传感器数据 Side Output 标签和 Kafka Sink 工厂已预留；OnlineInferenceJob 尚未把该 Side Output 接入 `invalid_sensor_topic`。
- PMML RUL 本地推理已接入；风险分类模型主要通过 REST 服务在线推理，分类模型 ONNX/TF Serving 在线链路仍属于扩展。

### 3. 未完成扩展（不含推荐系统和测试）

- ONNX Runtime 在线推理接入：尚未在 Flink 或模型服务中加载 ONNX 风险分类模型，也未完成 ONNX 模型导出、特征对齐、推理结果解析和配置切换。
- TF Serving 推理接入：尚未提供 TensorFlow SavedModel 导出、TF Serving 容器启动配置、REST/gRPC 调用客户端和在线推理链路切换。
...
- 无效传感器数据落 Kafka 链路：`INVALID_SENSOR_TAG` 和 `buildInvalidSensorSink()` 已存在，但 OnlineInferenceJob 尚未将无效数据侧输出写入 `invalid_sensor_topic`。
