...
- 人工审核标签回流基础链路；
- 规则 + 历史案例相似度维修推荐；
- Grafana 业务监控，系统级监控作为后续扩展。

## 当前实现状态

| 类别 | 内容 |
|---|---|
| 已完成 | Kafka 数据接入；Flink 清洗与窗口特征工程；REST 风险模型服务在线推理；PMML RUL 本地推理；Flink 在线推理；阈值告警；多规则特征异常告警；CEP 连续高风险告警；MySQL/Redis/Kafka 输出；在线特征快照；规则 + 历史案例相似度维修推荐；Spring Boot Dashboard API；轻量 Dashboard 页面；Grafana 业务 SQL 与 Dashboard JSON；人工审核回流基础链路；反馈训练样本生成；多模型训练报告与简单加权融合；MinIO 模型注册；风险模型服务热加载；增量训练闭环；Windows 一键启动脚本 |
| 部分完成 | Webhook 配置管理与推送容错；自动周期增量训练入口；Spring Boot Prometheus 指标暴露；模型注册后的人工激活流程。 |
| 未完成扩展| ONNX Runtime 在线推理接入；TF Serving 推理接入；Prometheus 系统级监控 Dashboard；模型灰度发布与自动回滚；生产级自动调度与热加载增强；Flink Async I/O REST 推理。 |
...
| 监控展示 | Grafana 业务看板；Prometheus 系统监控为后续扩展 | 展示业务指标、告警趋势和推荐结果 |
...
| 在线推理 | Flink 通过 REST 调用模型服务，PMML 本地推理部分完成，ONNX 为后续扩展 |
| 异常告警 | 高风险事件写入 Kafka、MySQL，并推送企业微信/钉钉 |
| 在线学习 | 人工审核标签回流基础链路，自动周期训练为后续扩展 |
| 推荐系统 | 根据高风险设备状态推荐维修策略、备件和相似案例 |
| 可视化展示 | 使用 Grafana 展示业务结果，Prometheus 系统级监控为后续扩展 |
...
- Webhook 已具备配置管理、测试发送、删除、风险级别过滤和 Flink 推送容错；真实企业微信/钉钉机器人地址需要部署时配置。
...
- Spring Boot 已暴露 Prometheus 指标入口；系统级 Prometheus 抓取、Flink/Kafka/JVM 指标采集和 Grafana 系统监控 Dashboard 尚未完成。
- PMML RUL 本地推理已接入；风险分类模型主要通过 REST 服务在线推理，分类模型 PMML/ONNX 在线链路仍属于扩展。
...
## 9. Grafana 业务看板与 Prometheus 扩展规划
