...
- 人工审核标签回流基础链路；
- 规则 + 历史案例相似度维修推荐；
- Grafana 业务监控；Prometheus 系统监控已有初版配置，完整采集闭环仍需补齐。

## 当前实现状态

| 类别 | 内容 |
|---|---|
| 已完成 | Kafka 数据接入；Flink 清洗与窗口特征工程；REST 风险模型服务在线推理；PMML RUL 本地推理；Flink 在线推理；阈值告警；多规则特征异常告警；CEP 连续高风险告警；MySQL/Redis/Kafka 输出；在线特征快照；规则 + 历史案例相似度维修推荐；Spring Boot Dashboard API；轻量 Dashboard 页面；Grafana 业务 SQL 与业务 Dashboard JSON；人工审核回流基础链路；反馈训练样本生成；多模型训练报告与简单加权融合；MinIO 模型注册；风险模型服务基础热加载；增量训练闭环；Windows 一键启动脚本 |
| 部分完成 | Webhook 配置管理与基础 HTTP 推送容错；自动周期增量训练入口；Spring Boot Prometheus 指标暴露；Prometheus 抓取配置与系统 Dashboard 初版；模型注册后的人工激活流程；无效传感器 Side Output 预留。 |
| 未完成扩展| ONNX Runtime 在线推理接入；TF Serving 推理接入；企业微信/钉钉 Webhook 真实适配；模型灰度发布与自动回滚；生产级自动调度与热加载增强；Flink Async I/O REST 推理；无效传感器数据落 Kafka 链路。 |
...
| 组件 | 推荐技术 | 在本项目中的作用 |
|---|---|---|
| 流处理引擎 | Apache Flink 1.15.2 | 实时清洗、窗口计算、CEP、在线推理 |
...
| 监控展示 | Grafana 业务看板；Prometheus 系统监控初版 | 展示业务指标、告警趋势；系统指标采集闭环仍需补齐 |
| 告警通知 | 通用 HTTP Webhook；企业微信 / 钉钉为待适配扩展 | 推送高风险设备告警 |
...
| 异常告警 | 高风险事件写入 Kafka、MySQL，并支持通用 HTTP Webhook 推送；企业微信/钉钉真实适配仍是扩展项 |
| 在线学习 | 人工审核标签回流基础链路，自动周期训练为后续扩展 |
| 推荐系统 | 根据高风险设备状态推荐维修策略、备件和相似案例 |
| 可视化展示 | 使用 Grafana 展示业务结果；Prometheus 系统监控已有初版配置，完整采集闭环仍需补齐 |
...
| Redis Sink | 缓存实时状态 |
| Kafka Sink | 写入异常事件流 |
| Webhook Sink | 通用 HTTP Webhook 推送；企业微信/钉钉真实格式仍待适配 |
...
- Webhook 已具备配置管理、测试发送、删除、风险级别过滤和 Flink 基础 HTTP 推送容错；企业微信/钉钉机器人消息格式、签名、安全重试和失败记录仍未完整适配。
...
- Prometheus 抓取配置和系统 Dashboard JSON 已有初版；Flink/Kafka/JVM/exporter 的真实采集闭环和部署文档仍未完整打通。
...
- 企业微信/钉钉 Webhook 真实适配：当前只有通用 HTTP JSON 推送和配置表读取，尚未完成企业微信/钉钉消息体格式、签名密钥、超时重试、失败落库和告警发送状态查询。
...
## 9. Grafana 业务看板与 Prometheus 系统监控初版

Grafana 业务看板已基于 MySQL 落地；Prometheus 抓取配置和系统 Dashboard JSON 已有初版，但 Flink、Kafka、JVM、主机 exporter 的真实采集闭环仍需补齐。
